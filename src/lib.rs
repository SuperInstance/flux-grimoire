...
#[cfg(test)]
mod tests {
    use crate::spell::{Spell, PatternType};
    ...
    fn make_grimoire() -> Grimoire {
        ...
        g.learn("s1", "Debug Print", PatternType::Debugging, "error", "println", "dev", "agent");
        g.learn("s2", "Deep Breath", PatternType::Behavioral, "panic", "breathe", "always", "agent");
        g.learn("s3", "Refactor", PatternType::Optimization, "slow", "profile", "perf", "agent");
        g
    }
    ...
    fn grimoire_prune() {
        let mut g = make_grimoire();
        for _ in 0..5 { g.record_outcome("s1", false); }
    ...
    fn curriculum_progress_empty() {
        let c = Curriculum::new(vec![
            Level { name: "L1".into(), spell_ids: vec!["s1".into()], min_confidence: 0.8 },
        ]);
        let g = Grimoire::new();
        let p = c.progress(&g);
    ...
    fn curriculum_progress_mastered() {
        let mut g = make_grimoire();
        for _ in 0..10 { g.record_outcome("s1", true); } // confidence = 1.0
        let c = Curriculum::new(vec![
            Level { name: "L1".into(), spell_ids: vec!["s1".into()], min_confidence: 0.9 },
            Level { name: "L2".into(), spell_ids: vec!["s2".into()], min_confidence: 0.8 },
        ]);
        let p = c.progress(&g);
